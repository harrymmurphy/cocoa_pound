# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 10:13:19 2024
...
# URLs for the data
ethanol_prices_url = 'https://raw.githubusercontent.com/harrymmurphy/sugar_crude/main/Ethanol%20Prices.csv'
sugar_prices_url = 'https://raw.githubusercontent.com/harrymmurphy/sugar_crude/main/Sugar_Prices.csv'
crude_prices_url = 'https://raw.githubusercontent.com/harrymmurphy/sugar_crude/main/Crude_Price.csv'

# Read data from URLs
ethanol_prices = pd.read_csv(ethanol_prices_url)
sugar_prices = pd.read_csv(sugar_prices_url)
crude_prices = pd.read_csv(crude_prices_url)

# Convert 'Date' columns to datetime
ethanol_prices['Date'] = pd.to_datetime(ethanol_prices['Date'])
sugar_prices['Date'] = pd.to_datetime(sugar_prices['Date'])
crude_prices['Date'] = pd.to_datetime(crude_prices['Date'])

# Assuming you want to work with weekly data, resample to weekly frequency
ethanol_prices_weekly = ethanol_prices.resample('W', on='Date').mean()
sugar_prices_weekly = sugar_prices.resample('W', on='Date').mean()
crude_prices_weekly = crude_prices.resample('W', on='Date').mean()

merged_data = pd.merge(ethanol_prices_weekly, sugar_prices_weekly, on='Date', how='inner', suffixes=('_ethanol', '_sugar'))
merged_data = pd.merge(merged_data, crude_prices_weekly, on='Date', how='inner')
merged_data.rename(columns={'Anhydrous Price ': 'Anhydrous Price', 'Hydrous Price': 'Hydrous Price', 'Price_x': 'Sugar Price', 'Price_y': 'Crude Price'}, inplace=True)

merged_data['Ethanol Price'] = merged_data[['Anhydrous Price', 'Hydrous Price']].mean(axis=1)

log_merged_data = merged_data.apply(lambda x: np.log10(x) if np.issubdtype(x.dtype, np.number) else x)
...
for col, val in zip(log_merged_data.columns, out):
...
log_merged_data['Row'] = np.arange(len(log_merged_data))

log_merged_data['Row'] = range(len(log_merged_data))



log_merged_data['Ethanol Price'].rolling(6).corr(log_merged_data['Sugar Price'])

# formatting the output for One Month
k = 1
for i, j in enumerate(log_merged_data['Ethanol Price'].rolling(6).corr(log_merged_data['Sugar Price'])):
    if (i >= 3 and i < 1100):
        print(f'The correlation in from Ethanol to Sugar during months\
        {k} through {i+1} is {j}')
        i = 0
        k += 1

rolling_window_sizes = [3, 6, 12, 24]  # Adjust window sizes as needed
rolling_correlations = {}
for window in rolling_window_sizes:
    rolling_correlations[f'Ethanol_to_Sugar_{window}'] = log_merged_data['Ethanol Price'].rolling(window).corr(log_merged_data['Sugar Price'])

# Construct a time series dataframe
rolling_correlations_df = pd.DataFrame(rolling_correlations, index=log_merged_data.index)
rolling_correlations_df_filtered = rolling_correlations_df.dropna()
print(rolling_correlations_df_filtered)

high_correlatory_breaks = rolling_correlations_df[rolling_correlations_df_filtered > 0.75]

# Drop rows where all values are NaN (if any)
high_correlatory_breaks = high_correlatory_breaks.dropna(how='all')
target_dates = rolling_correlations_df.iloc[:, 0]

high_correlation_dates = rolling_correlations_df.index[rolling_correlations_df['Ethanol_to_Sugar_6'] > 0.75]

merged_data_with_correlations = pd.merge(merged_data, rolling_correlations_df, left_index=True, right_index=True)

# Filter merged_data based on the rolling correlations condition (> 0.75)
filtered_data = merged_data_with_correlations[merged_data_with_correlations['Ethanol_to_Sugar_6'] > 0.75]

# Extract original sugar and ethanol prices from filtered_data
filtered_prices = filtered_data[['Sugar Price', 'Ethanol Price']]

print(filtered_prices.head())

average_sugar_price = filtered_data['Sugar Price'].mean()
average_ethanol_price = filtered_data['Ethanol Price'].mean() 

# Calculate ratio between average Ethanol Price and Sugar Price
ethanol_to_sugar_ratio =  average_sugar_price / average_ethanol_price 

print(f"The average Ethanol to Sugar ratio is: {ethanol_to_sugar_ratio}")
